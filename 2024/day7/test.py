...
def check_possible_equations(a: int, numbers: List[int]) -> Dict[str, any]:
    """
    Check if any equation formed by permutations of the first three elements
    of the input list and the equations x + y + z, x * y + z, x + y * z, x * y * z
    evaluates to the target `a`.
    """
    # Generate all permutations of three numbers
    ...
    for perm in perms:
        ...
        if a in equations:
            return {
                "Found": True,
                # "Matching Permutation": perm,
                # "Matching Equation Index": equations.index(a)
            }
    return {
        "Found": False,
        # "Matching Permutation": None,
        # "Matching Equation Index": None
    }

results = []
for target, nums in data:
    result = check_possible_equations(target, nums)
